...
void print(Blob* blob, const std::string &name) {
	const auto tensor = caffe2::BlobGetMutableTensor(blob, DeviceType::CPU);
	const auto data = tensor->data<float>();
	const auto dataVec = std::vector<float>(data, data + tensor->size());

	cout << name << " (" << tensor->dim()
			<< "): " << dataVec << endl;
}

void run() {
	cout << "## Toy Regression ##" << endl;
	Workspace workspace;
